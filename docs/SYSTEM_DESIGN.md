...
| UIフォント | Google Fontsの`Noto Sans JP`（外部配信） |
| ゲーム描画 | Phaser 3 |
...
| `pages/_document.tsx` | HTML言語属性、Noto Sans JPの外部読み込み |
...
| `listRooms` | なし | 部屋一覧取得 |
| `createRoom` | 部屋名、モード、難易度、人数、ステージ、任意パスコード | 部屋作成 |
| `joinRoom` | 部屋ID、任意パスコード、応答 | 部屋参加と入室認証 |
| `leaveRoom` | なし | 退出 |
| `startGame` | なし | ホストによる開始 |
| `removePlayer` | プレイヤーID | ホストによる待機中プレイヤー退出 |
| `endGame` | なし | ホストによる試合終了 |
| `prepareRematch` | なし | CPUと結果を消去して再戦待機へ戻す |
| `setTeam` | チーム番号 | 開始前のチーム変更 |
| `setColor` | カラー | 開始前の色変更 |
| `setSpectator` | 観戦フラグ、応答 | 開始前の出走・観戦変更 |
| `retire` | なし | 出走中プレイヤーのリタイア |
...
| `removedFromRoom` | 理由 | 退出対象をロビーへ戻す |
...
- 部屋パスコードはルームID別にソルト付き`scrypt`ハッシュで保持し、平文やハッシュをクライアントへ配信しません。
...
- 人間プレイヤー全員に`finishedAt`または`retiredAt`が設定された場合、または`timeoutAt`到達で終了します。
- 人間の出走者がいる場合、CPUは終了条件の「全員」判定から除外します。人間の出走者が0人の場合はCPU全員のゴールも終了条件です。
- ゴール済み人間プレイヤーのクライアントは、未ゴールプレイヤーをカメラ対象にします。
- 観戦対象の変更はクライアント表示のみで、ゲーム状態へ影響しません。
- ホスト終了時は`finishReason`を`hostEnded`として結果を確定します。
- 再戦準備ではCPU、ゴール状態、タイマー、勝者を消去し、接続中の人間プレイヤーを初期位置へ戻します。
- 大会中継モードの観戦対象はホストが手動で切り替えます。
- ライブ順位はゴール順を優先し、未ゴール選手を高度順に並べます。
- `spectator`は待機中のみ変更でき、出走枠が満員の場合は出走への変更を拒否します。試合開始処理が役割を固定してCPU数を確定します。
- 観戦者は物理演算、順位、リザルトから除外します。人間の出走者がいれば人間全員のゴール、出走者が0人ならCPU全員のゴール、またはタイムアウト・ホスト終了で試合を終了します。

### 15.1 リザルトCSV

- ホストの結果画面だけにダウンロード操作を表示します。
- CSVはクライアントで生成し、サーバーへ保存しません。
- Excelでの文字化けを避けるためUTF-8 BOMを付与します。
- プレイヤー名などの先頭が数式記号の場合は、表計算ソフトの数式として実行されないよう無効化します。
